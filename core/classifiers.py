...
class BERTSubclassPredictor:
    ...
    def __init__(self):
        ...
        self.vocab_file = join(pretrained_path, 'vocab.txt')
        self.model_file = join(models_dir, 'cpcs_classify.json')
        self.weights_file = join(models_dir, 'cpcs_classify.h5')
        self.subclass_codes_file = join(models_dir, 'cpcs_classify.subclass_codes.txt')
        self.SEQ_LEN = 128
    ...
    def _load_subclass_codes(self):                  
        ...
        """
        with open(self.subclass_codes_file, 'r') as file:
    ...
    def _load_dictionary(self):
        ...
        with codecs.open(vocab_file, 'r', 'utf8') as reader:
            for line in reader:
                token = line.strip()
                self.tokens_dict[token] = len(tokens_dict)
